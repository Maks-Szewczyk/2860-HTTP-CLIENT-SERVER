...
def recv_line(sock: socket.socket, max_len: int = MAX_FILENAME_LEN) -> bytes:
    ...
    while True:
        ...
        data.extend(sock.recv(BUFSIZE))
        print(data)
        if(len(data) > max_len):
            raise ValueError("data length is greater than maximum length allowed")
        else: 
            try:
                n_index = data.index(ord('\n'))
                print("found index")
                data.pop(n_index)
            except:
...
def run_server(port: int, outdir: str, ipv6: bool) -> None:
    ...
    while True:
        ...
        print("calling handle_client")
        handle_client(connectionSocket,outdir)
...
def run_client(server_ip: str, port: int, file_path: str, ipv6: bool) -> int:
    ...
    print("filename sent!")
    if (recv_line(client_socket) == b"OK"): 
        client_socket.send(struct.pack('!Q', file_size))
        with open (file_path, 'rb' ) as f:
            while True:
                chunk = f.read(BUFSIZE)
                if (chunk == b""): break
                else: client_socket.send(chunk)
    else:
        sys.exit(1)
    if(recv_line(client_socket) == b"OK"):
        print("chunks sent client")
        sys.exit(0)
    else: sys.exit(255)
